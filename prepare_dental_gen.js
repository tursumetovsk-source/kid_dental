...
<script>
  (() => {
    ...
    // Older educational state (5-7 years)
    ...
    function highlightGameTool(name) {
      ...
      const id = name === 'brush' ? 'dgToolBrush' : (name === 'water' ? 'dgToolWater' : (name === 'apple' ? 'dgToolApple' : (name === 'puzzle' ? 'dgToolPuzzle' : (name === 'memory' ? 'dgToolMemory' : 'dgToolSorter'))));
    ...
    function setGameMode(mode) {
      ...
      if (tabToddler && tabOlder) {
        if (mode === 'toddler') {
          ...
          if (badge) badge.textContent = "🧩 РАЗВИВАЮЩИЕ ИГРЫ (5–7 ЛЕТ)";
          if (title) title.textContent = "Умная Зубная Академия!";
        }
      }
    ...
    function startEmbeddedStep1() {
      ...
      if (text) text.textContent = '1. Почисти пятнышки щёточкой! (Тапай на монстриков)';

      const tooth = document.getElementById("dgGameToothChar");
      if (tooth) tooth.style.display = 'block';

      const olderStage = document.getElementById("dgOlderEduStage");
    ...
    // ==========================================
    // 🧩 OLDER KIDS EDUCATIONAL GAMES (5-7 лет: Пазлы, Мемори, Сортировка)
    // ==========================================
    const puzzlePiecesData = [
      { id: "p1", slot: "s1", icon: "👑", label: "Корона", hint: "Верх слева" },
      { id: "p2", slot: "s2", icon: "👓", label: "Глазки", hint: "Верх справа" },
      { id: "p3", slot: "s3", icon: "💎", label: "Элайнеры", hint: "Низ слева" },
      { id: "p4", slot: "s4", icon: "🛡️", label: "Защита", hint: "Низ справа" }
    ];
    ...
    function startOlderStep1() {
      ...
      highlightGameTool('puzzle');

      const s1 = document.getElementById("dgGameStep1");
      const s2 = document.getElementById("dgGameStep2");
      const s3 = document.getElementById("dgGameStep3");
      if (s1) { s1.className = 'step-pill active'; s1.innerHTML = '<span>🧩</span> <span>1. Пазл</span>'; }
      if (s2) { s2.className = 'step-pill'; s2.innerHTML = '<span>🧠</span> <span>2. Мемори</span>'; }
      if (s3) { s3.className = 'step-pill'; s3.innerHTML = '<span>🍎</span> <span>3. Сортировка</span>'; }

      const emoji = document.getElementById("dgGameHintEmoji");
      const text = document.getElementById("dgGameHintText");
      if (emoji) emoji.textContent = '🧩';
      if (text) text.textContent = '1. Собери кусочки пазла в рамочку! (Тапай на детальки)';
      ...
      const olderStage = document.getElementById("dgOlderEduStage");
      if (!olderStage) return;
      olderStage.style.display = 'flex';

      // Render Puzzle Stage
      olderStage.innerHTML =
      ...
      const shuffledPieces = [...puzzlePiecesData].sort(() => Math.random() - 0.5);
      ...
      document.querySelectorAll(".puzzle-slot").forEach(slotEl => {
        const slotHandler = (e) => {
          ...
          // If no piece selected yet, find the piece that belongs to this slot
          if (!pieceToPlace || pieceToPlace.slot !== slotId) {
            const match = puzzlePiecesData.find(p => p.slot === slotId);
          ...
          if (pieceToPlace && pieceToPlace.slot === slotId) {
            ...
            slotEl.innerHTML = '<span style="font-size: 32px; line-height: 1;">' + pieceToPlace.icon + '</span><span class="puzzle-slot__label" style="color: #7048c4;">' + pieceToPlace.label + '</span>';
            ...
            const lx = rect.left - sRect.left + (rect.width || 60) / 2;
            const ly = rect.top - sRect.top + (rect.height || 60) / 2;
            ...
            if (puzzlePiecesLeft <= 0) {
              ...
              if (s1) {
                s1.className = 'step-pill completed';
                s1.innerHTML = '<span>✅</span> <span>1. Пазл собран!</span>';
              }
              setTimeout(() => { startOlderStep2(); }, 750);
            }
          }
        };
    ...
    function startOlderStep2() {
      ...
      const text = document.getElementById("dgGameHintText");
      if (emoji) emoji.textContent = '🧠';
      if (text) text.textContent = '2. Мемори: найди все одинаковые пары карточек!';
      ...
      olderStage.innerHTML =
        '<div class="memory-container" id="dgMemoryContainer">' +
          '<div class="memory-grid" id="dgMemoryGrid"></div>' +
        '</div>';
      ...
      memoryCards.forEach((c, idx) => {
        ...
        card.innerHTML =
          '<div class="memory-card__inner">' +
            ...
            '<div class="memory-card__front">' + c.icon + '</div>' +
          '</div>';
          ...
          if (memoryFlippedCards.length === 2) {
            ...
            if (c1.dataset.sym === c2.dataset.sym) {
              ...
              setTimeout(() => {
                ...
                const stage = document.getElementById("dgGameStage");
                const sRect = stage ? stage.getBoundingClientRect() : { left: 0, top: 0, width: 300, height: 260 };
                const lx = rect.left - sRect.left + 20;
                const ly = rect.top - sRect.top + 20;

                triggerGameParticles(lx, ly, 16, 'laser');
                showGameSparkleFloat(lx, ly, '⭐ ПАРА!');
                ...
                if (memoryMatchedPairs === 4) {
                  ...
                  if (s2) {
                    s2.className = 'step-pill completed';
                    s2.innerHTML = '<span>✅</span> <span>2. Память 100%</span>';
                  }
                  setTimeout(() => { startOlderStep3(); }, 850);
                }
              }, 250);
            } else {
              // No match
    ...
    function startOlderStep3() {
      ...
      const text = document.getElementById("dgGameHintText");
      if (emoji) emoji.textContent = '🍎';
      if (text) text.textContent = '3. Сортировка: собери 3 полезных продукта для крепких зубок!';
      ...
      foods.forEach(f => {
        ...
        const foodHandler = (e) => {
          ...
          if (f.isHealthy) {
            ...
            const stage = document.getElementById("dgGameStage");
            const sRect = stage ? stage.getBoundingClientRect() : { left: 0, top: 0, width: 300, height: 260 };
            const lx = rect.left - sRect.left + 30;
            const ly = rect.top - sRect.top + 20;

            triggerGameParticles(lx, ly, 18, 'bubble');
            showGameSparkleFloat(lx, ly, '🧺 ' + f.icon);
            ...
            if (sorterHealthyCollected >= 3) {
              if (s3) {
                s3.className = 'step-pill completed';
                s3.innerHTML = '<span>✅</span> <span>3. Эксперт!</span>';
              }
              onOlderGameVictory();
            }
          } else {
            ...
            showGameSparkleFloat(lx, ly, f.tip);
          }
        };
    ...
    function onOlderGameVictory() {
      setTimeout(() => {
        ...
        const vicSub = document.getElementById("dgVictorySub");
        if (vicIcon) vicIcon.textContent = "🏆✨🎓🦷🥇";
        if (vicTitle) vicTitle.textContent = "УРА! ТЫ — ЗНАТОК ЗДОРОВОЙ УЛЫБКИ!";
        if (vicSub) vicSub.textContent = "Ты решил все головоломки, развил супер-память и логику! Твои зубки под 100% защитой! 🌟";
          ...
          '<div class="dg-shell">' +
            '<div class="dg-game-teaser__card">' +
              '<div class="dg-game-teaser__left">' +
                ...
                '<p>Обучающие игры по возрасту: чистите зубик щёточкой для самых маленьких или собирайте развивающие пазлы, тренируйте память и изучайте полезные продукты для будущих школьников!</p>' +
                '<div class="dg-game-teaser__buttons">' +
                  ...
                  '<button class="dg-game-teaser__btn dg-game-teaser__btn--older dg-open-game-btn" data-game-mode="older" type="button"><span>🧩</span> <span>5–7 лет: Пазлы и логика</span></button>' +
                '</div>' +
              '</div>' +
          ...
          '<div class="dg-game-modal__container">' +
            ...
            '<div class="game-wrapper">' +
              '<header class="game-header">' +
                '<div class="game-badge">🧩 РАЗВИВАЮЩИЕ ИГРЫ (5–7 ЛЕТ)</div>' +
                '<h2 class="game-title">Умная Зубная Академия!</h2>' +
              ...
              '<div class="game-mode-tabs" role="tablist">' +
                ...
                '<button class="mode-tab-btn active" id="dgModeTabOlder" type="button" data-mode="older"><span>🧩 5–7 лет (Пазлы)</span></button>' +
              '</div>' +
              '<div class="game-card">' +
                '<div class="step-tracker" role="tablist">' +
                  '<div class="step-pill active" id="dgGameStep1"><span>🧩</span> <span>1. Пазл</span></div>' +
                  '<div class="step-pill" id="dgGameStep2"><span>🧠</span> <span>2. Мемори</span></div>' +
                  '<div class="step-pill" id="dgGameStep3"><span>🍎</span> <span>3. Сортировка</span></div>' +
                '</div>' +
                '<div class="hint-banner" id="dgGameHintBanner">' +
                  '<span id="dgGameHintEmoji">🧩</span> <span id="dgGameHintText">1. Собери кусочки пазла в рамочку!</span>' +
                '</div>' +
                '<div class="stage" id="dgGameStage">' +
                  ...
                  '<svg class="tooth-svg" id="dgGameToothChar" viewBox="0 0 240 260" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: none;">' +
                    ...
                    '</g>' +
                  '</svg>' +
                  '<!-- Older Educational Stage (Puzzle, Memory, Sorter) -->' +
                  '<div class="stage-older-edu" id="dgOlderEduStage"></div>' +
                ...
                '</div>' +
                '<!-- Older Tools Palette -->' +
                '<div class="tools-palette" id="dgOlderToolsPalette" role="toolbar" aria-label="Развивающие игры">' +
                  '<button class="tool-btn puzzle-btn selected" id="dgToolPuzzle" aria-label="Пазл"><span class="tool-icon">🧩</span><span class="tool-label">1. ПАЗЛ</span></button>' +
                  '<button class="tool-btn memory-btn" id="dgToolMemory" aria-label="Мемори"><span class="tool-icon">🧠</span><span class="tool-label">2. МЕМОРИ</span></button>' +
                  '<button class="tool-btn sorter-btn" id="dgToolSorter" aria-label="Сортировка"><span class="tool-icon">🍎</span><span class="tool-label">3. СОРТИРОВКА</span></button>' +
                '</div>' +
                '<!-- Victory Overlay -->' +
                '<div class="victory-overlay" id="dgGameVictoryModal">' +
                  '<div class="victory-tooth" id="dgVictoryIcon">🏆✨🎓🦷🥇</div>' +
                  '<h3 class="victory-title" id="dgVictoryTitle">УРА! ТЫ — ЗНАТОК ЗДОРОВОЙ УЛЫБКИ!</h3>' +
                  '<p class="victory-sub" id="dgVictorySub">Ты решил все головоломки, развил супер-память и логику! Твои зубки под 100% защитой! 🌟</p>' +
